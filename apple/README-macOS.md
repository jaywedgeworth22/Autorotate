# Autorotate — macOS App

The macOS deliverable for Autorotate: a SwiftUI app (macOS 14+, Swift 5.9) that
wraps the tested [`AutorotateCore`](../AutorotateCore) rotation engine with a main
...
```
Autorotate-macOS/
├── AutorotateMacApp.swift        @main: WindowGroup (NavigationSplitView sidebar:
...
│                              @ModelActor stores implementing AutorotateCore's
...
├── KeychainInventory.swift    Settings-screen listing of com.autorotate Keychain
...
│                              scheduler interval
└── AutorotateMac.entitlements    Sandbox + user-selected read-write + bookmarks +
...
- **Keychain Sharing** — `$(AppIdentifierPrefix)com.autorotate.shared` in
...
- Connector admin credentials and the Infisical Universal Auth clientSecret
  ...
  (`com.autorotate.credential.<connectorId>.<secretId>` /
  `com.autorotate.infisical.<workspaceId>`), `kSecAttrAccessibleAfterFirstUnlock`.
