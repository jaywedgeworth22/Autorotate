# HANDOFF — Autorotate fleet onboarding (from KIMI, quota-limited)

**Date:** 2026-08-20 · **From:** KIMI (Kimi Work session) · **To:** any local seat (Grok / Claude / Codex / …)
**Slack `repo:`:** `Autorotate`, `ai-fleet-coordinator`, `fleet-infra` · **Acronym:** `TS`
...
1. **GitHub repo live** — `jaywedgeworth22/Autorotate`, **public**, `main` pushed.
   ...
   `~/Code/Autorotate`: `origin` = GitHub, `bundle` = old local bundle (kept as backup).
2. **Fleet script ran** — `onboard-new-app.sh --repo Autorotate --acronym TS …` from lane
   `~/apps/fleet-kimi`. Created: live board `~/apps/AUTOROTATE-EFFORT-LOG.md`, grok lane
   `~/apps/autorotate-grok` (branch `grok/fleet-onboard`), `~/Code/copilot-worktrees/Autorotate`,
   `fleet-apps.json` row.
3. **Autorotate-side bootstrap** — branch `kimi/fleet-onboard`, lane `~/apps/autorotate-kimi`,
...
4. **Coordinator registries** — branch `kimi/autorotate-onboard`, lane `~/apps/fleet-kimi`,
...
5. **Live `~/apps` lockstep edits applied directly** (not via git): AGENT-SYNC.md,
   ...
   (`autorotate` entry: bundleId `com.autorotate.ios`, scheme `Autorotate-iOS`, pre-TestFlight note).
6. **CI first-run repairs** — commits `168863c` + `71f81c3` on PR #16:
   ...
   - iOS compile fixes: `AddTargetView.fileSection` view → `fileTargetSection` (collided with
     ...
     `AutorotateApp.init()` marked `@MainActor`, engine wiring uses a local `keychain` constant
     (definite-initialization order).
   - Untracked generated `apple/Autorotate.xcodeproj/` and added it to .gitignore (xcodegen output).

## In progress / state at handoff

- **iOS scheme build (`Autorotate-iOS`) was mid-verification locally.** Last full build
  (`/tmp/autorotate-ios-build.log`) failed only on the AppModel definite-init error, which
  commit `168863c` then fixed — **the fixed code has not been rebuilt yet**. Resume:
  ```bash
  cd ~/apps/autorotate-kimi/apple
  USER=jay xcodegen generate   # USER env must be set; sandbox shells may lack it
  USER=jay xcodebuild -project Autorotate.xcodeproj -scheme Autorotate-iOS \
...
- **macOS scheme never attempted** (`Autorotate-macOS`, `-destination 'platform=macOS'`).
- **Web fix unverified** — no local `npm ci && npm run check && npm run build` run yet;
  CI on PR #16 will exercise it on the next push.
- Dependabot opened PRs #1–15 on Autorotate (actions + radix bumps) — triage after #16 lands.
...
4. `gh workflow run "Effort Issues Sync" --repo jaywedgeworth22/Autorotate` (workflow_dispatch)
   so the first board rows become GitHub issues.
5. `git -C ~/Code/Autorotate pull --ff-only` (integration tree fast-forward; code-main-keeper
   would also catch it).
6. Move rows to Completed on `~/apps/AUTOROTATE-EFFORT-LOG.md` + repo mirror, and update the
...
8. Apple Notes `[TS, FLEET, <Agent>] onboard Autorotate` in folder Coding via
...
- Branch protection on Autorotate `main`: require PR + checks `web`, `apple`, `gitleaks`.
...
- Autorotate hard invariants (AGENTS.md): zero plaintext persistence; append-only hash-chained
  audit; 6-step pipeline; `docs/architecture.md` capability matrix is source of truth;
  FK columns `bigint unsigned` in Drizzle.
- Fleet rules: never edit in `~/Code/Autorotate` (use `~/apps/autorotate-<seat>` lanes);
  board-first claim/closeout; light theme default — **note:** `AutorotateApp.swift` currently
...
- `~/Code/Autorotate/_ref/` contains untracked reference copies made during onboarding —
