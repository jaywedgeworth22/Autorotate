...
## In Progress
- **Apache-2.0 + Kimi dump backup + catalog fold-in** — CURSOR · branch `cursor/kimi-apache-merge` · worktree `~/apps/topspin-cursor-kimi-apache`. Relicense to Apache 2.0 (© Jay). Backup Kimi dump + Secret Rotator nickname (`app/`). Fold Grok extra catalog into live web + TopSpinCore.
