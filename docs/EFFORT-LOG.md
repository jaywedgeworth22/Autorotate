...
## In Progress
- **Full internal rename TopSpin → Autorotate** — GROK · issue #59 · branch `autorotate/full-rename-topspin`. Repo already renamed on GitHub. Live tree still has directory/package names (`TopSpin-iOS`, `TopSpinCore`, `api/topspin`, `contracts/topspin`, env `TOPSPIN_*`, etc.). Phase 1: agent placeholder + router import path started. Remaining requires directory moves + import/env updates (prefer Mac seat with `git mv` + sed for Apple packages).
- **Owner: Developer portal App IDs for Autorotate** — CURSOR leftover from Grok.  Git IDs on AG [#48](https://github.com/jaywedgeworth22/Autorotate/pull/48).  #50 stays closed.  Product https://autorotate.codes.  Agents cannot create portal IDs.
